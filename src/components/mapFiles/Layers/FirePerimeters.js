...
export default function FirePerimeterLayer({year, perimeterData}) {
  console.log('rendering fire perimter layer', year)
  ...
  // };
    ...
    // return useMemo(()=>{
      if(perimeterData){
        console.log('I have perimeter data')
        return (
          <MapVectorLayer
            source={
              vector({
                features: new GeoJSON().readFeatures(perimeterData, {
                  dataProjection : 'EPSG:4326', 
                  featureProjection: 'EPSG:3857'
                }),
              })}
            style={config['firePerimeters'].styleFunction}
        />
        );
      }
      else{
        console.log('i have no perimeter data')
        return(<></>)
      }  
    // },[year, perimeterData])
...
export const MemoizedPerimeterLayer = React.memo(FirePerimeterLayer)
